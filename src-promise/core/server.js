...
//exposing this server object should be a little more strict on the httpServer object
var _httpServer;
var _httpServerHandler;
...
var run = function(env) {
  var configWatcher, restart, start, stop;
  _httpServer = null;
  _httpServerHandler = null;
  ...
  stop = function() {
    if (_httpServer != null) {
      ...
      _httpServerHandler.destroy();
      _httpServerHandler = undefined;
      env.reset();
    }
  };
  start = function() {
    return Promise.coroutine(function*(){
      if(_httpServer || _httpServerHandler) {
        throw Error("server has already started");
      }
      yield env.loadPlugins();
      _httpServerHandler = yield setup(env);
      _httpServer = http.createServer(_httpServerHandler);
      enableDestroy(_httpServer);
      var defer = Promise.defer();
      _httpServer.on('error', function(error) {
        env.emit('serverError', error);
        defer.reject(error);
      });
      _httpServer.on('listening', function() {
        env.emit('serverListening');
        defer.resolve();
      });
      var retVal = _httpServer.listen(env.config.port, env.config.hostname);
      yield defer.promise;
  ...
  .then(function(_httpServer){
    ...
    env.emit("runFinished", _httpServer);
    return _httpServer;
  })
};
